...
class RestoreBackupScreen(
        ...
        Scaffold(
            ...
            LazyColumnWithAction(
                ...
                if (state.canRestore) {
                    ...
                    if (results != null && (results.hasModHistoryCategories || results.hasModHistoryGroups || results.hasModLinkedSources || results.hasModUpdateWatch)) {
                        item {
                            SectionCard(MR.strings.mod_data) {
                                if (results.hasModHistoryCategories) {
                                    Option(RestoreOptions.modOptions[0], state, viewModel)
                                }
                                if (results.hasModHistoryGroups) {
                                    Option(RestoreOptions.modOptions[1], state, viewModel)
                                }
                                if (results.hasModLinkedSources) {
                                    Option(RestoreOptions.modOptions[2], state, viewModel)
                                }
                                if (results.hasModUpdateWatch) {
                                    Option(RestoreOptions.modOptions[3], state, viewModel)
                                }
                            }
                        }
                    }
    ...
    @Composable
    private fun Option(
        option: RestoreOptions.Entry,
        state: RestoreBackupViewModel.State,
...
class RestoreBackupViewModel(
    ...
    private fun setError(error: Any?, canRestore: Boolean, results: BackupFileValidator.Results?) {
        mutableState.update {
            it.copy(
                error = error,
